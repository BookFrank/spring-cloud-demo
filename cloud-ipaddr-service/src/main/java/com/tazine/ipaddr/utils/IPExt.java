...
//        IPExt.loadAsStream(IPExt.class.getResourceAsStream("mydata4vipday1.datx"));
    ...
    public static void loadAsStream(InputStream in) {
        lastModifyTime = System.currentTimeMillis();
        lock.lock();
        try {
            dataBuffer = ByteBuffer.wrap(getBytesByStream(in));
            dataBuffer.position(0);
            offset = dataBuffer.getInt(); // indexLength
            byte[] indexBytes = new byte[offset];
            dataBuffer.get(indexBytes, 0, offset - 4);
    ...
    private static byte[] getBytesByStream(InputStream in) {
        byte[] bs = null;
        try {
            bs = new byte[in.available()];
            int readBytesLength = 0;
            int i = in.available();
            in.read(bs, readBytesLength, i);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return bs;
